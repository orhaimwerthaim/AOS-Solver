...
std::discrete_distribution<> Bp::Environment_discrete_dist1{0.5,0,0,0.5}; //AOS.SampleDiscrete(tCell,{0.5,0,0,0.5})

State* Bp::CreateStartState(string tyep) const {
...
void Bp::CheckPreconditions(const BpState& state, double &reward, bool &__meetPrecondition, int actionId) const
    {
        ...
        __meetPrecondition = true;
            if(actType == navigateAction)
            {
                ...
                if(!__meetPrecondition) reward += -100;
            }
    }
...
void Bp::ExtrinsicChangesDynamicModel(const BpState& state, BpState& state_, double rand_num, int actionId, double& reward, const int &__moduleExecutionTime)  const
{
    ActionType &actType = ActionManager::actions[actionId]->actionType;
}

void Bp::ModuleDynamicModel(const BpState &state, const BpState &state_, BpState &state__, double rand_num, int actionId, double &__reward, OBS_TYPE &observation, const int &__moduleExecutionTime) const
{
    ActionType &actType = ActionManager::actions[actionId]->actionType;
    observation = -1;
    ...
    if(actType == navigateAction)
    {
        ...
        if(oDirection==Down){agentNextLocation=agentLocation==L10?L00:agentLocation==L11?L01:OUT;
        };
        if(oDirection==Up){agentNextLocation=agentLocation==L00?L10:agentLocation==L01?L11:OUT;
        };
        if(oDirection==Left){agentNextLocation=agentLocation==L01?L00:agentLocation==L11?L10:OUT;
        };
        if(oDirection==Right){agentNextLocation=agentLocation==L10?L11:agentLocation==L00?L01:OUT;
        };
...
bool Bp::ProcessSpecialStates(const BpState &state, double &reward) const
{
    bool isFinalState = false;
    if (state.agentOneLoc == L01 && state.agentTwoLoc == L10)
    {
        reward += 100;
        isFinalState = true;
    }
