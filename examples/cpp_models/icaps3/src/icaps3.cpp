 ...
 *Icaps3Belief class
 * ==============================================================================*/
int Icaps3Belief::num_particles = 5000;


Icaps3Belief::Icaps3Belief(vector<State*> particles, const DSPOMDP* model,
	Belief* prior) :
	ParticleBelief(particles, model, prior),
	icaps3_(static_cast<const Icaps3*>(model)) {
}
...
{
	Icaps3ResponseModuleAndTempEnums eObs = (Icaps3ResponseModuleAndTempEnums)obs;
	return enum_map_icaps3::vecResponseEnumToString[eObs]; 
}
    std::string Prints::PrintState(Icaps3State state)
    {
        stringstream ss;
        ss << "STATE: ";
 	...
 	std::string Icaps3::GetActionDescription(int actionId) const
	 {
...
void Icaps3Belief::Update(int actionId, OBS_TYPE obs, std::map<std::string,bool> updates) {
	...
	while (updated.size() < num_particles && trials < 10 * num_particles) {
		State* particle = icaps3_->Copy(particles_[cur]);
		bool terminal = icaps3_->Step(*particle, Random::RANDOM.NextDouble(),
			actionId, reward, o);
 
		if (!terminal && o == obs) 
			{
				Icaps3State &icaps3_particle = static_cast<Icaps3State &>(*particle);
				if(!Globals::IsInternalSimulation() && updates.size() > 0)
				{
					Icaps3State::SetAnyValueLinks(&icaps3_particle);
					map<std::string, bool>::iterator it;
					for (it = updates.begin(); it != updates.end(); it++)
					{
						*(icaps3_particle.anyValueUpdateDic[it->first]) = it->second; 
					} 
				}
				updated.push_back(particle);
		} else {
			icaps3_->Free(particle);
		}
...
int Icaps3::NumActions() const {
	return ActionManager::actions.size();
}

double Icaps3::ObsProb(OBS_TYPE obs, const State& state, int actionId) const {
	return 0.9;
}
...
std::default_random_engine Icaps3::generator;

std::normal_distribution<double>  Icaps3::place_normal_dist1(30000,15000); //AOS.SampleNormal(30000,15000)
std::normal_distribution<double>  Icaps3::observe_normal_dist2(15000,2000); //AOS.SampleNormal(15000,2000)
std::normal_distribution<double>  Icaps3::pick_normal_dist3(40000,10000); //AOS.SampleNormal(40000,10000)
std::normal_distribution<double>  Icaps3::navigate_normal_dist4(40000,10000); //AOS.SampleNormal(40000,10000)
std::discrete_distribution<> Icaps3::place_discrete_dist1{0.8,0.01,0}; //AOS.SampleDiscrete(enumRealCase,{0.8,0.01,0})
std::discrete_distribution<> Icaps3::pick_discrete_dist2{0.8,0.1,0,0.1}; //AOS.SampleDiscrete(enumRealCase,{0.8,0.1,0,0.1})
std::discrete_distribution<> Icaps3::navigate_discrete_dist3{0.8,0.2}; //AOS.SampleDiscrete(enumRealCase,{0.8,0.2})
std::discrete_distribution<> Icaps3::Environment_discrete_dist4{0.04,0,0.6}; //AOS.SampleDiscrete(tDiscreteLocation,{0.04,0,0.6})

State* Icaps3::CreateStartState(string tyep) const {
    Icaps3State* startState = memory_pool_.Allocate();
    Icaps3State& state = *startState;
    ...
    {
        ActionManager::Init(const_cast <Icaps3State*> (startState));
    }
    return startState;
}
...
Belief* Icaps3::InitialBelief(const State* start, string type) const {
	int N = Icaps3Belief::num_particles;
	...
	return new Icaps3Belief(particles, this);
}
 

 

POMCPPrior* Icaps3::CreatePOMCPPrior(string name) const { 
		return new UniformPOMCPPrior(this);
}

void Icaps3::PrintState(const State& state, ostream& ostr) const {
	const Icaps3State& farstate = static_cast<const Icaps3State&>(state);
	if (ostr)
		ostr << Prints::PrintState(farstate);
}

void Icaps3::PrintObs(const State& state, OBS_TYPE observation,
	ostream& ostr) const {
	const Icaps3State& farstate = static_cast<const Icaps3State&>(state);
...
void Icaps3::PrintBelief(const Belief& belief, ostream& out) const {
	 out << "called PrintBelief(): b printed"<<endl;
		out << endl;
	
}

void Icaps3::PrintAction(int actionId, ostream& out) const {
	out << Prints::PrintActionDescription(ActionManager::actions[actionId]) << endl;
}

State* Icaps3::Allocate(int state_id, double weight) const {
	Icaps3State* state = memory_pool_.Allocate();
	state->state_id = state_id;
	state->weight = weight;
	return state;
}

State* Icaps3::Copy(const State* particle) const {
	Icaps3State* state = memory_pool_.Allocate();
	*state = *static_cast<const Icaps3State*>(particle);
	state->SetAllocated();
	return state;
}

void Icaps3::Free(State* particle) const {
	memory_pool_.Free(static_cast<Icaps3State*>(particle));
}

int Icaps3::NumActiveParticles() const {
	return memory_pool_.num_allocated();
}

bool Icaps3::Step(State& s_state__, double rand_num, int actionId, double& reward,
	...
	Icaps3State &state__ = static_cast<Icaps3State &>(s_state__);
	 logd << "[Icaps3::Step] Selected Action:" << Prints::PrintActionDescription(ActionManager::actions[actionId]) << "||State"<< Prints::PrintState(state__);
	...
	Icaps3State &state = static_cast<Icaps3State &>(*s_state);
	...
	Icaps3State &state_ = static_cast<Icaps3State &>(*s_state_);
...
void Icaps3::CheckPreconditions(const Icaps3State& state, double &reward, bool &meetPrecondition, int actionId) const
    {
...
void Icaps3::SampleModuleExecutionTime(const Icaps3State& farstate, double rand_num, int actionId, int &__moduleExecutionTime) const
{
    ActionType &actType = ActionManager::actions[actionId]->actionType;
    if(actType == placeAction)
    {
        __moduleExecutionTime=Icaps3::place_normal_dist1(Icaps3::generator);
    }
    if(actType == observeAction)
    {
        __moduleExecutionTime=Icaps3::observe_normal_dist2(Icaps3::generator);
    }
    if(actType == pickAction)
    {
        __moduleExecutionTime=Icaps3::pick_normal_dist3(Icaps3::generator);
    }
    if(actType == navigateAction)
    {
        __moduleExecutionTime=Icaps3::pick_normal_dist3(Icaps3::generator);
...
void Icaps3::ExtrinsicChangesDynamicModel(const Icaps3State& state, Icaps3State& state_, double rand_num, int actionId, double& reward, const int &__moduleExecutionTime)  const
{
    ActionType &actType = ActionManager::actions[actionId]->actionType;
}

void Icaps3::ModuleDynamicModel(const Icaps3State &state, const Icaps3State &state_, Icaps3State &state__, double rand_num, int actionId, double &__reward, OBS_TYPE &observation, const int &__moduleExecutionTime) const
{
    ActionType &actType = ActionManager::actions[actionId]->actionType;
    observation = -1;
    ...
    if(actType == placeAction)
    {
        Icaps3ResponseModuleAndTempEnums  realCase;
        realCase=(Icaps3ResponseModuleAndTempEnums)(place_enumRealCase + 1 + Icaps3::place_discrete_dist1(Icaps3::generator));
    ...
    {
        Icaps3ResponseModuleAndTempEnums  realCase;
        realCase=(Icaps3ResponseModuleAndTempEnums)(pick_enumRealCase + 1 + Icaps3::pick_discrete_dist2(Icaps3::generator));
    ...
    if(actType == navigateAction)
    {
        ...
        Icaps3ResponseModuleAndTempEnums  realCase;
        realCase=(Icaps3ResponseModuleAndTempEnums)(navigate_enumRealCase + 1 + Icaps3::navigate_discrete_dist3(Icaps3::generator));
...
bool Icaps3::ProcessSpecialStates(const Icaps3State &state, double &reward) const
{
...
std::string Icaps3::PrintObs(int action, OBS_TYPE obs) const 
{
	return Prints::PrintObs(action, obs);
}

std::string Icaps3::PrintStateStr(const State &state) const { return ""; };
}// namespace despot
