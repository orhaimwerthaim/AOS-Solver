...
State* Icaps::CreateStartState(string tyep) const {
	...
	state.cupDiscreteGeneralLocation = state.locationNear_elevator1.discrete_location; //state.tDiscreteLocationObjects[discrete_dist1(generator)];
...
void Icaps::ExtrinsicChangesDynamicModel(const IcapsState& state, IcapsState& state_, double rand_num, int actionId, double& reward,
								   ...
								   OBS_TYPE &observation, const int &__moduleExecutionTime) const
	{
		...
		if (ActionManager::actions[actionId]->actionType == observeAction)
		{
			...
			if (realCase == observed && AOSUtils::Bernoulli(0.9)) {state__.cupAccurateLocation = true;__moduleResponse = eSuccess;} else{ state__.cupAccurateLocation = false; __moduleResponse = eFailed;}
