...
void ActionManager::Init(IrosState* state)
{
    ...
    ActionDescription *observe_holding_can = new ActionDescription;
    observe_holding_can->actionType = observe_holding_canAction;
    observe_holding_can->actionId = id++;
    ActionManager::actions.push_back(observe_holding_can);

    ActionDescription *observe_arm_outstretched = new ActionDescription;
    observe_arm_outstretched->actionType = observe_arm_outstretchedAction;
    observe_arm_outstretched->actionId = id++;
    ActionManager::actions.push_back(observe_arm_outstretched);

    ActionDescription *basic_pick = new ActionDescription;
    basic_pick->actionType = basic_pickAction;
    basic_pick->actionId = id++;
    ActionManager::actions.push_back(basic_pick);
...
std::string Prints::PrintObs(int action, int obs)
{
    ...
    std::string Prints::PrintActionType(ActionType actType)
    {
        ...
        case observe_holding_canAction:
            return "observe_holding_canAction";
        case observe_arm_outstretchedAction:
            return "observe_arm_outstretchedAction";
        case basic_pickAction:
            return "basic_pickAction";
        }
    }
