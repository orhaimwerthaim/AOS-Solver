...
#include <string>
namespace despot { 
    void ActionDescription::SetActionParametersByState(Icaps2State *state, std::vector<std::string> indexes){}
    std::vector<ActionDescription*> ActionManager::actions;


void NavigateActionDescription::SetActionParametersByState(Icaps2State *state, std::vector<std::string> indexes)
{
...
void ActionManager::Init(Icaps2State* state)
{
...
/*eOutside_lab211,
		...
		eNear_elevator1,
		eUnknown*/
    NavigateActionDescription a = NavigateActionDescription(tLocation(eOutside_lab211, true), "", "discrete_location:eOutside_lab211,actual_location:true")
	...
	{
        MongoDB_Bridge::RegisterAction(ActionManager::actions[j]->actionId, enum_map_icaps2::vecActionTypeEnumToString[ActionManager::actions[j]->actionType], ActionManager::actions[j]->GetActionParametersJson_ForActionRegistration());
    }
}
}